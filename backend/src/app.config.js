...
function allowListedFileFilter(req, file, cb) {
  const allowedTypes = /jpeg|jpg|png|gif|pdf|doc|docx|xls|xlsx|ods|ppt|pptx|txt|md|zip|rar/i;
  const extname = allowedTypes.test(path.extname(file.originalname).toLowerCase());

  if (extname || !path.extname(file.originalname)) {
    return cb(null, true);
  } else {
    cb(new Error(`Ungültiger Dateityp: ${file.originalname}`));
  }
}
